...
class TestAdvanced:
    def test_get_first_response(self):
        ...
        time.sleep(
            1
        )  # send interval; put before assertions to make sure the send interval passes
    ...
    def test_wait_for_response(self):
        ...
        # send data
        send_time = time.time()
        data = {"data": [5, 6, 7, 8, send_time], "ending": "\n", "concatenate": ";"}
        ...
        data_get_back = {"response": s, "strip": True}
    ...
    def test_send_for_response(self):
        ...
        s = f"abcd{time.time()}"
        data = {"response": f'Got: "{s}"', "data": s, "ending": "\n", "strip": True}
